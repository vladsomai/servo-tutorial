...
async function SendDataToSerialPort() {
    ...
    const COMMAND_UPPER_LMT_BYTE_SIZE = 4;
    const COMMAND_LOWER_LMT_BYTE_SIZE = 4;
    ...
    const upper_rotation_limit = 0;
    const lower_rotation_limit = 0;

    //convert input values to motor units
    const lower_lmt_motor_units = get4BytesFromNumber(
        lower_rotation_limit * MOVE_DISPLACEMENT_MOTOR_UNITS_PER_ROTATION
    );

    const upper_lmt_motor_units = get4BytesFromNumber(
        upper_rotation_limit * MOVE_DISPLACEMENT_MOTOR_UNITS_PER_ROTATION
    ); 
    ...
    const bytes = new Uint8Array(
            ...
            COMMAND_UPPER_LMT_BYTE_SIZE +
            COMMAND_LOWER_LMT_BYTE_SIZE
    );

    bytes.set([alias]);
    bytes.set([motor_command], 1);
    bytes.set([motor_command_length], 2);
    bytes.set(lower_lmt_motor_units, 3);
    bytes.set(upper_lmt_motor_units, 7);
