...
int main(int argc, char* argv[])
{
    ...
    uint8_t alias = 255;
    uint8_t motor_command = 30;
    #define motor_command_length 8
    double upper_rotation_limit = 0;
    double lower_rotation_limit = 0;
    ...
    double lower_lmt_motor_units = lower_rotation_limit * (double)MOVE_DISPLACEMENT_MOTOR_UNITS_PER_ROTATION;
    uint32_t transmitLowerLmt = ConvertDoubleToUint32LittleEndian(lower_lmt_motor_units);
    uint8_t bufferLowerLmt[4] = { 0 };
    ConvertUintTo4BytesArr(bufferLowerLmt, sizeof(uint32_t), transmitLowerLmt);

    for (int i = 0; i < sizeof(uint32_t); i++)
    {
        cmd[alias_cmd_len + i] = bufferLowerLmt[i];
    }

    double upper_lmt_motor_units = upper_rotation_limit * (double)MOVE_DISPLACEMENT_MOTOR_UNITS_PER_ROTATION;
    uint32_t transmitUpperLmt = ConvertDoubleToUint32LittleEndian(upper_lmt_motor_units);
    uint8_t bufferUpperLmt[4] = { 0 };
    ConvertUintTo4BytesArr(bufferUpperLmt, sizeof(uint32_t), transmitUpperLmt);

    for (int i = 0; i < sizeof(uint32_t); i++)
    {
        cmd[alias_cmd_len + sizeof(uint32_t) + i] = bufferUpperLmt[i];
    }
