...
#define motor_command_length 4
    ...
    double internal_acceleration = (acceleration_rpm2 / (double)pow(60, 2)) * ((double)MOVE_DISPLACEMENT_MOTOR_UNITS_PER_ROTATION / (double)pow(MOVE_TIME_MOTOR_UNITS_PER_SECOND, 2)) * pow(2, 32);
    double communication_acceleration = internal_acceleration / pow(2, 8);

    uint32_t transmitCommAcc = ConvertDoubleToUint32LittleEndian(communication_acceleration);

    uint8_t bufferTransmitCommAcc[4] = { 0 };

    ConvertUintTo4BytesArr(bufferTransmitCommAcc, sizeof(uint32_t), transmitCommAcc);
...
#define alias_cmd_len 3
    ...
    {
        cmd[alias_cmd_len + i] = bufferTransmitCommAcc[i];
    }
