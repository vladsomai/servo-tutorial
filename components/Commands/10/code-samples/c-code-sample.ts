...
uint64_t ConvertUint64ToUint64LittleEndian(const uint64_t input);
void ConvertUintTo6BytesArr(uint8_t* buffer, uint8_t buffSize, uint64_t number);
...
int main(int argc, char* argv[])
{
    ...
    uint8_t alias = 255;
    uint8_t motor_command = 10;
#define motor_command_length 6
    uint64_t elapsed_time_since_reset = 0;

	uint64_t elapsedTimeLittleEndian = ConvertUint64ToUint64LittleEndian(elapsed_time_since_reset);

	uint8_t bufferTransmitTime[motor_command_length] = { 0 };
	ConvertUintTo6BytesArr(bufferTransmitTime, motor_command_length, elapsedTimeLittleEndian);
...
#define alias_cmd_len 3
	...
	{
		cmd[alias_cmd_len + i] = bufferTransmitTime[i];
	}
...
uint64_t ConvertUint64ToUint64LittleEndian(const uint64_t input)
{
	...
	result |= (input & bmask64[0]) << (BITS_PER_BYTE * 7);
	result |= (input & bmask64[1]) << (BITS_PER_BYTE * 5);
	result |= (input & bmask64[2]) << (BITS_PER_BYTE * 3);
	result |= (input & bmask64[3]) << (BITS_PER_BYTE * 1);
	result |= (input & bmask64[4]) >> (BITS_PER_BYTE * 1);
	result |= (input & bmask64[5]) >> (BITS_PER_BYTE * 3);
	result |= (input & bmask64[6]) >> (BITS_PER_BYTE * 5);
	result |= (input & bmask64[7]) >> (BITS_PER_BYTE * 7);
...
void ConvertUintTo6BytesArr(uint8_t* buffer, uint8_t buffSize, uint64_t number)
{
	if (buffSize != 6)
	{
		//assure the buffer size is 6 bytes
		return;
	}

	for (int i = 7, j = 0; i >= 2; i--, j++)
	{
		uint64_t maskedNum = (uint64_t)(number & bmask64[i]);
		buffer[j] = (uint8_t)((maskedNum) >> (i * 8));
	}
}
