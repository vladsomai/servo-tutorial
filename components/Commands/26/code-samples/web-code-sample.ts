...
async function SendDataToSerialPort() {
    ...
    const alias = 255;
    const motor_command = 26;
    const motor_command_length = 8;
    const velocity_rpm = 0
    ...
    const bytes = new Uint8Array(
            ...
            COMMAND_VELOCITY_SIZE +
            COMMAND_MOVE_SIZE
    );

    //convert input values to motor units
    const internal_velocity = (velocity_rpm / 60) * (MOVE_DISPLACEMENT_MOTOR_UNITS_PER_ROTATION / MOVE_TIME_MOTOR_UNITS_PER_SECOND) * (2 ** 32)
    const communication_velocity = get4BytesFromNumber(internal_velocity / (2 ** 12))
    ...
    bytes.set(communication_velocity, 3);
