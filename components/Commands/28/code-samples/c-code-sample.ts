...
uint16_t ConvertDoubleToUint16LittleEndian(const uint16_t input);
void ConvertUintTo2BytesArr(uint8_t* buffer, uint8_t buffSize, uint16_t number);
...
int main(int argc, char* argv[])
{
    ...
    uint8_t alias = 255;
    uint8_t motor_command = 28;
    #define motor_command_length 4
    uint16_t motor_current = 0;
    uint16_t regeneration_current = 0;

    uint16_t motorCurrentLittleEndian = ConvertDoubleToUint16LittleEndian(motor_current);
    uint8_t bufferMotorCurrent[2] = { 0 };
    ConvertUintTo2BytesArr(bufferMotorCurrent, sizeof(uint16_t), motorCurrentLittleEndian);

    uint16_t regenCurrentLittleEndian = ConvertDoubleToUint16LittleEndian(regeneration_current);
    uint8_t bufferRegenCurrent[2] = { 0 };
    ConvertUintTo2BytesArr(bufferRegenCurrent, sizeof(uint16_t), regenCurrentLittleEndian);
    ...
    {
        cmd[alias_cmd_len + i] = bufferMotorCurrent[i];
    }

    for (int i = 0; i < sizeof(uint16_t); i++)
    {
        cmd[alias_cmd_len + sizeof(uint16_t) + i] = bufferRegenCurrent[i];
    }
...
uint16_t ConvertDoubleToUint16LittleEndian(const uint16_t input)
{
	...
	result |= (input & bmask64[0]) << 8;
	result |= (input & bmask64[1]) >> 8;
...
void ConvertUintTo2BytesArr(uint8_t* buffer, uint8_t buffSize, uint16_t number)
{
	...
	{
		//assure the buffer size is 2 bytes
		return;
	}

	for (int i = buffSize - 1, j = 0; i >= 0; i--, j++)
	{
		buffer[j] = (number & bmask64[i]) >> (i * 8);
	}
}
