...
#define motor_command_length 8
    double move_displacement_rotations = 0;
    ...
    double move_displacement_motor_units = move_displacement_rotations * (double)MOVE_DISPLACEMENT_MOTOR_UNITS_PER_ROTATION;
    double move_time_motor_units = move_time_seconds * (double)MOVE_TIME_MOTOR_UNITS_PER_SECOND;

    uint32_t transmitDisplacement = ConvertDoubleToUint32LittleEndian(move_displacement_motor_units);
    uint32_t transmitTime = ConvertDoubleToUint32LittleEndian(move_time_motor_units);

    uint8_t bufferDisplacement[4] = { 0 };
    uint8_t bufferTime[4] = { 0 };

    ConvertUintTo4BytesArr(bufferDisplacement, sizeof(uint32_t), transmitDisplacement);
...
#define alias_cmd_len 3
    ...
    {
        cmd[alias_cmd_len + i] = bufferDisplacement[i];
    }
