  ...
  */
    ...
    const execute_command = async () => {
        ...
        const execute_programming = async () => {
            ...
            const totalPagesToFlash =
                remainingBytesToFlash.length / FLASH_PAGE_SIZE;
            ...
            for (
                ...
                remainingBytesToFlash =
                    remainingBytesToFlash.slice(FLASH_PAGE_SIZE);
            }
        };
    ...
    return (
        <>
            ...
            <article className="mb-5 prose prose-slate max-w-full">
                <div className="flex justify-center">
                    <h2>
                        Let&apos;s learn how to implement the firmware upgrade
                        command
                    </h2>
                </div>
                <h3>Preparing the firmware</h3>
                <p>
                    Firstly, we must understand how the firmware file is
                    structured, here is a sample:{" "}
                </p>
                <Image
                ...
                <ul>
                    <li>
                        First 8 bytes represent the model code. In the following
                        guide, we will refer to those 8 bytes as{" "}
                        <b>model code</b>.
                    </li>
                    <li>
                        9th byte represent the firmware compatibility code. In
                        the following guide, we will refer to this byte as{" "}
                        <b>fw compatibility code</b>.
                    </li>
                    <li>Bytes 10 to 13 are currently not used.</li>
                    <li>
                        The rest of the bytes from the firmware file represent
                        the application code. All the following operations will
                        involve manipulating this byte array(from 14th byte to
                        the end of the firmware file). In the following guide,
                        we will refer to those bytes as <b>fw app code</b>.
                    </li>
                </ul>
                <h4>Flash memory mapping</h4>
                <p>
                    {" "}
                    The flash memory is organized in <b>pages</b> each
                    containing <b>2048 bytes.</b>
                </p>
                <p>
                    The microcontroller has 31 flash pages available for the
                ...
                <p>
                    Pages 1 to 5 and 31 will be set by the manufacturer of the
                    motor, you must not write at those memory addreses yourself.
                    To summerize, the memory map looks like this:
                </p>
                ...
                <h4>
                    Steps to follow in preparing the firmware flashing process
                </h4>
                <ol>
                    <li>Load the firmware file.</li>
                    <li>
                        Split the firmware file into 4 uint8_t arrays containing
                        all the bullets described above in the structure of the
                        file.
                    </li>
                    <li>
                        Verify the application code is not larger than the
                        maximum flash size reserved for application area.
                    </li>
                    <li>
                        Compute the firmware size in machine <b>words</b>. This
                        is achieved by getting the length of the{" "}
                        <b>fw app code</b> and dividing it by 4. The firmware
                        size than needs to be stored into a uint32_t with little
                        endian format.
                    </li>
                    <li>
                        Compute the CRC32 using the <b>fw app code</b>. You can
                        find exapmples on the web on how to implement a CRC32
                        function. The CRC32 value needs to be stored into an
                        uint32_t with little endian format.
                    </li>
                    <li>
                        Compute the final firmware that you will use to send
                        over the serial port. The final firmware must have the
                        first 4 bytes the ones computed at step 4(the ones
                        stored in little endian format), the <b>fw app code</b>{" "}
                        and the CRC32 bytes computed at step 5. Keep in mind the
                        following rules:
                    </li>
                    <ul>
                        <li>
                            Calculate the current size of the final firmware by
                            adding up the firmware size bytes(4bytes / 32bits),
                            fw app code size(variable in length), CRC32
                            bytes(4bytes / 32bits). We will call this size{" "}
                            <b>fw size without padding</b>
                        </li>
                        <li>
                            You will need to concatenate &apos;0&apos; bytes at
                            the end, until a full flash page is completed.
                            Generate a new uint8_t array full of &apos;0&apos;
                            with the size equal to{" "}
                            <i> ceil(currentFwSize / 2048)</i>
                        </li>
                    </ul>
                    <li>
                        At this point you shall have the following uint8_t
                        array: fw_size + fw_app_code + CRC32 + zeros until the
                        end of a page. We will call this the{" "}
                        <b>final fw app code</b>
                    </li>
                </ol>
                <h2>Executing the flashing process</h2>
                <p>
                    Using the final array constructed above, we will need to
                    break it up into chunks of 2048 bytes where you will also
                    append the axis, command and length bytes. We will call this
                    array the <b>telegram</b>.
                </p>
                ...
                <ol>
                    ...
                    <li>
                        Bytes 3,4: Flash page size(8+1+1+2048), where 8 is size
                        of model code, 1 is size of fw compatibility code, 1 is
                        page number that is currently beeing written to, 2048 is
                        the flash page size.
                    </li>
                    <li>Bytes 5 to 12: Model code.</li>
                    <li>Byte 13: Fw compatibility code.</li>
                    <li>
                        Byte 14: The current page number that is being written
                        to (this is a uint8_t and range from 5 to 29).
                    </li>
                    <li>
                        Bytes 15 to 2062: The 2048 bytes chunk from the final fw
                        app code
                    </li>
                </ol>
                <h4>Execution</h4>
                <ol>
                    ...
                    <li>
                        Construct telegram until all 2048 chunks are sent over
                        the serial port to the motor, using a delay of 50ms
                        between all the telegrams.
                    </li>
                    <li>Do a firmware reset (command 27).</li>
                </ol>
            </article>
            <div className="flex justify-around w-100 my-10">
                <div className="flex flex-col justify-center items-center">
                    <Image
                        className="rounded-box h-[200px] w-auto"
                        width={200}
                        height={200}
                        src="/motor_m1.png"
