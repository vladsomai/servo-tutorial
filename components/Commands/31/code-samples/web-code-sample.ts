    ...
    while (serialPort?.readable) {
        ...
        try {
            while (true) {
                ...
                {
                    //the ping command should return 13 bytes, last 10 are payload
                    
                    receivedDataPelem.innerHTML += '<br/>'
                    
                    for(let i=3; i<value.length; i++)
                    {
                        receivedDataPelem.innerHTML += String.fromCharCode(value.at(i))
                    }
                }
            }
        } catch (error) {
...
async function SendDataToSerialPort() {
    ...
    const COMMAND_PING_LENGTH_BYTE_SIZE = 10;

    // Define all the parameters for this command
    const alias = 255;
    const motor_command = 0;
    const motor_command_length = 10;
    const ping_text = ''

    if (ping_text.length != 10)
    {
        const errorMsg = "You must input exactly 10 characters!";
        console.log(errorMsg);
        sentDataPelem.innerHTML = errorMsg;
        return;
    }
    ...
    const byteOffset = 3;
    for(let i = 0; i<ping_text.length; i++)
    {
        const asciiCode = ping_text.charCodeAt(i)
        bytes.set([asciiCode], byteOffset + i)
    }
