...
void ConvertAsciiStrToUint8Arr(uint8_t* buffer, uint8_t buffSize, const char* uniqueId);
...
int main(int argc, char* argv[])
{
    ...
    uint8_t alias = 255;
    uint8_t motor_command = 21;
    #define motor_command_length 10
    const char* ping_text = "";
    
    if (strlen(ping_text) != 10)
    {
        printf("You must input exactly 10 characters!\\n");
        return -1;
    }

    uint8_t bufferPing[motor_command_length] = { 0 };
    ConvertAsciiStrToUint8Arr(bufferPing, 10, ping_text);
    ...
    {
        cmd[alias_cmd_len + i] = bufferPing[i];
    }
    ...
    else
    {
        ...
        printf("\\n");
        printf("Received payload converted to string: ");
        for (int i = 3; i < recvSize; i++)
        {
...
void ConvertAsciiStrToUint8Arr(uint8_t* buffer, uint8_t buffSize, const char* inputStr)
{
	size_t inputSize = strlen(inputStr);

	if (buffSize != inputSize)
	{
		return;
	}
