...
export default async function handler(
  ...
  try {
    ...
    for (let _file_ in files) {
      ...
      //@ts-ignore
      zip.file(file.filepath.slice(file.filepath.lastIndexOf('\\')), fileBuffer)
    ...
    const zipPath = receivedFiles + "\\" + fields.attachmentID + '.zip'
    ...
    zip.generateNodeStream({ type: "nodebuffer", streamFiles: true })
      ...
      .on('finish', async () => {//when the zip finished being written on disk 
        ...
        await uploadAttachment(fileBuffer, zipPath.slice(zipPath.lastIndexOf('\\')))
